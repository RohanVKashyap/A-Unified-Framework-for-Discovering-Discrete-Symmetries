...
def main(args):
    ...
    np.random.seed(1024)
    #ts = 64      #Original, Always use ts = 64
    ts = args.ts  # Only for review 
    vs = 480
    d = 10
    divisors = [1, 2, 5, 10]
    k = args.k
    ...
    def create_data(d, k, batch_size, true_indices):
        ...
        # Additional data
        #c_train_ds, d_train_ds, p_train_ds = get_data_v1(train_ds[0:5], true_indices) #Comment only for review, Use it
    ...
    for i in np.arange(arm_iterations):
        ...
        #np.save(f'c_{k}_10_ts_{ts}_dec_rewards.npy',np.array(all_rewards))
        #np.save(f'c_{k}_10_y_ts_{ts}_dec_hat.npy',np.array(all_y_hat))
        #np.save(f'c_{k}_10__ts_{ts}_dec_mu_hat.npy',np.array(all_mu_hat))
        #np.save(f'c_{k}_10__ts_{ts}_dec_train_indices.npy',np.array(all_train_indices))
