...
def main(args):
    ...
    train_y = train_y + n_eps*np.random.normal(0, np.std(train_y), train_y.shape)
    val_y = val_y + n_eps*np.random.normal(0, np.std(train_y), val_y.shape)
    print('k:',k)
    print('n_eps:',n_eps)
    ...
    for i in np.arange(arm_iterations):
        ...
        np.save(f'c_{k}_10_{n_eps}_gaussian_rewards.npy',np.array(all_rewards))
        np.save(f'c_{k}_10_{n_eps}_gaussian_y_hat.npy',np.array(all_y_hat))
        np.save(f'c_{k}_10_{n_eps}_gaussian_mu_hat.npy',np.array(all_mu_hat))
        np.save(f'c_{k}_10_{n_eps}_gaussian_train_indices.npy',np.array(all_train_indices))
        np.save(f'c_{k}_10_{n_eps}_gaussian_subgroup_indices.npy',np.array(all_subgroup_indices))
...
if __name__=='__main__':
    ...
    parser.add_argument('--neps',type=float)
